module JifeUncertaintyQuantification
...
"""
	mutable struct StochasticJifeParameter
...
- `stochparams::Vector{`[`StochasticJifeParameter`](@ref)`}` : Vector defining the stochastic state space.
...
mutable struct StochasticOGSModelParams
	...
	stochparams::Vector{StochasticJifeParameter}
	samplemethod::Type
...
function JifeUQParams(file_stochasticmodelparams::String, file_samplemethodparams::String)
	...
	return JifeUQParams(stochasticmodelparams, samplemethodparams)
end

"""
	abstract type AbstractJifeUQ 

Supertype for stochastic OGS6 models. Implemented by [`JifeUQASG`](@ref).
"""
abstract type AbstractJifeUQ end 

"""
	abstract type AbstractJifeUQMonteCarlo 

Supertype for stochastic OGS6 models. Implemented by [`JifeUQMC`](@ref).
"""
abstract type AbstractJifeUQMonteCarlo <: AbstractJifeUQ end

"""
	abstract type AbstractJifeUQSensitivity 

Supertype for stochastic OGS6 models. Implemented by [`JifeUQMCSobol`](@ref) and [`JifeUQMCMorris`](@ref).
""" 
abstract type AbstractJifeUQSensitivity <: AbstractJifeUQMonteCarlo end 

"""
	mutable struct JifeUQASG
...
- `ogsuqparams::`[`JifeUQParams`](@ref) : stochastic model and sample method parameters.
...
"""
mutable struct JifeUQASG <: AbstractJifeUQ
	ogsuqparams::JifeUQParams
	asg::AdaptiveHierarchicalSparseGrid
end

"""
	mutable struct JifeUQMC
...
"""
mutable struct JifeUQMC <: AbstractJifeUQMonteCarlo
	ogsuqparams::JifeUQParams
	mc::MonteCarlo
end

"""
	mutable struct JifeUQMCSobol
...
"""
mutable struct JifeUQMCSobol <: AbstractJifeUQSensitivity
	ogsuqparams::JifeUQParams
	mc::MonteCarloSobol
end
...
"""
mutable struct JifeUQMCMorris <: AbstractJifeUQSensitivity
	ogsuqparams::JifeUQParams
	mc::MonteCarloMorris
end

include("./JifeUncertaintyQuantification/convienence.jl")
include("./JifeUncertaintyQuantification/utils.jl")
include("./JifeUncertaintyQuantification/utils_user_file.jl")
include("./JifeUncertaintyQuantification/utils_xdmf.jl")
include("./JifeUncertaintyQuantification/utils_asg.jl")
include("./JifeUncertaintyQuantification/utils_sobol.jl")
include("./JifeUncertaintyQuantification/utils_integrity_criteria.jl")
include("./JifeUncertaintyQuantification/utils_print.jl")

function init(::Type{AdaptiveHierarchicalSparseGrid}, ogsuqparams::JifeUQParams)
	...
	return JifeUQASG(ogsuqparams, asg)
end

function init(::Type{MonteCarlo}, ogsuqparams::JifeUQParams)
	N = ogsuqparams.samplemethodparams.N
	CT = ogsuqparams.samplemethodparams.CT
	RT = ogsuqparams.samplemethodparams.RT
	...
	return JifeUQMC(ogsuqparams, mc)
end

function init(::Type{MonteCarloSobol}, ogsuqparams::JifeUQParams)
	N = ogsuqparams.samplemethodparams.N
	CT = ogsuqparams.samplemethodparams.CT
	RT = ogsuqparams.samplemethodparams.RT
	nshots = ogsuqparams.samplemethodparams.nshots
	...
	return JifeUQMCSobol(ogsuqparams, mc)
end

function init(::Type{MonteCarloMorris}, ogsuqparams::JifeUQParams)
	N = ogsuqparams.samplemethodparams.N
	CT = ogsuqparams.samplemethodparams.CT
	RT = ogsuqparams.samplemethodparams.RT
	...
	return JifeUQMCMorris(ogsuqparams, mc)
end

"""
	init(ogsuqparams::JifeUQParams)

Helper function to instantiate a stochastic OGS6 model. Return an object of type [`JifeUQASG`](@ref), [`JifeUQMC`](@ref), [`JifeUQMCSobol`](@ref), or [`JifeUQMCMorris`](@ref).

# Arguments
- `ogsuqparams::`[`JifeUQParams`](@ref): Stochastic model parameters.
"""
function init(ogsuqparams::JifeUQParams)
...
# Arguments
- `rt::T` : Scaling weight of [`DistributedSparseGrids.HierarchicalCollocationPoint`](https://baxmittens.github.io/DistributedSparseGrids.jl/dev/lib/lib/#DistributedSparseGrids.HierarchicalCollocationPoint). `T` is returntype of [`fun`](https://github.com/baxmittens/JifeUncertaintyQuantification.jl/blob/5a8efaadb8b9de9e2380d759b2dd5e129550497a/src/JifeUncertaintyQuantification/user_function_template.jl#L40) calling the OGS6 binary.
- `tolrt::T` : Average scaling weight weighted by [`SparseGridParams`](@ref).tol. `T` is returntype of [`fun`](https://github.com/baxmittens/JifeUncertaintyQuantification.jl/blob/5a8efaadb8b9de9e2380d759b2dd5e129550497a/src/JifeUncertaintyQuantification/user_function_template.jl#L40) calling the OGS6 binary.
...
"""
	start!(ogsuqasg::JifeUQASG, refinetohyperedges=false)

Starts the adaptive sparse grid sampling procedure.

# Arguments
- `ogsuqasg::`[`JifeUQASG`](@ref): Stochastic OGS adaptive sparse grid model.
- `refinetohypercube::Bool`: If set to true the sparse grid is refined such that all vertices of the n-cube spawn.
"""
function start!(ogsuqasg::JifeUQASG, refinetohypercube=false)
	asg = ogsuqasg.asg
...
# Arguments
- `ogsuqmc::`[`JifeUQMC`](@ref): Stochastic OGS Monte Carlo model.
"""
function start!(ogsuqmc::JifeUQMC)
	DistributedMonteCarlo.load!(ogsuqmc.mc, ogsuqmc.ogsuqparams.stochasticmodelparams.ogsparams.outputpath)
	return nothing
end

"""
	start!(ogsuqmc::JifeUQMCSobol)

Starts the Monte Carlo Sobol sampling.

# Arguments
- `ogsuqmc::`[`JifeUQMCSobol`](@ref): Stochastic OGS Monte Carlo Sobol model.
"""
function start!(ogsuqmc::JifeUQMCSobol)
	DistributedMonteCarlo.load!(ogsuqmc.mc, ogsuqmc.ogsuqparams.stochasticmodelparams.ogsparams.outputpath)
	return DistributedMonteCarlo.distributed_Sobol_Vars(ogsuqmc.mc, Main.fun, workers())
end

"""
	start!(ogsuqmc::JifeUQMCMorris)

Starts the Monte Carlo Sobol sampling.

# Arguments
- `ogsuqmc::`[`JifeUQMCMorris`](@ref): Stochastic OGS Monte Carlo Morris model.
"""
function start!(ogsuqmc::JifeUQMCMorris)
	DistributedMonteCarlo.load!(ogsuqmc.mc, ogsuqmc.ogsuqparams.stochasticmodelparams.ogsparams.outputpath)
	return DistributedMonteCarlo.distributed_means(ogsuqmc.mc, Main.fun, workers())
end

function exp_val_func(x,ID,ogsuqasg::JifeUQASG,retval_proto::RT) where {RT}
...
function var_func(x,ID,ogsuqasg::JifeUQASG, exp_val::RT) where {RT}
...
- `ogsuqasg::`[`JifeUQASG`](@ref): Stochastic OGS adaptive sparse grid model.
"""
function variance(ogsuqasg::JifeUQASG,exp_val::RT) where {RT}
...
- `ogsuqmc::`[`JifeUQMC`](@ref): Stochastic OGS Monte Carlo model.
"""
function variance(ogsuqmc::JifeUQMC, exp_val::RT) where {RT}
	return distributed_var(ogsuqmc.mc, Main.fun, exp_val, workers())
end

export OGS6ProjectParams, StochasticJifeParameter, StochasticOGSModelParams, SampleMethodParams, SparseGridParams, MonteCarloParams, MonteCarloSobolParams,
	JifeUQParams, generatePossibleStochasticParameters, generateStochasticOGSModell, generateSampleMethodModel, loadStochasticParameters, 
	JifeUQASG, JifeUQMC, JifeUQMCSobol, JifeUQMCMorris, AdaptiveHierarchicalSparseGrid, Normal, Uniform, JifeModelDef, getAllPathesbyAttribute!, VTUFile, rename!, AHSG, 
