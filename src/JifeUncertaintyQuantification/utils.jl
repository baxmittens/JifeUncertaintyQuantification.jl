...
- `keywords::Vector{String}`: List of [`keywords`](https://github.com/baxmittens/JifeUncertaintyQuantification.jl/blob/6faab955f69da653e568b61eeb8890040001e7e6/src/JifeUncertaintyQuantification/utils.jl#L2).
"""
function generatePossibleStochasticParameters(
	projectfile::String, 
	file::String="./StochasticParameters.xml", 
	keywords::Vector{String}=jife_numeric_keyvals
	)

	modeldef = read(JifeModelDef,projectfile)
	stochparams = Vector{StochasticJifeParameter}()
	pathes = Vector{String}()
	for keyword in keywords
		getAllPathesbyAttribute!(pathes,modeldef.xmlroot,keyword)
	end
...
function generateStochasticOGSModell(
	...
	for path in stochpathes
		...
		el = getElementbyPath(modeldef, string(splitpath[1]))
		val = getAttribute(el, string(splitpath[2]))
		valspec = 1
		...
		dist = Uniform(lb,ub)
		user_function = x->x
		push!(stochparams, StochasticJifeParameter(path,valspec,dist,lb,ub))
	end

	ogs6pp = OGS6ProjectParams(projectfile,simcall,additionalprojecfilespath,outputpath,postprocfile)
	#sogs = JifeUQParams(ogs6pp,stochparams,stochmethod,n_local_workers,remote_workers,sogsfile)
...
"""
	CPtoStoch(x::T,stoparam::StochasticJifeParameter) where T<:Number
...
"""
function CPtoStoch(x::T,stoparam::StochasticJifeParameter) where T<:Number
	return lin_func(x, -1.0, stoparam.lower_bound, 1.0, stoparam.upper_bound)
end

"""
	StochtoCP(x::T,stoparam::StochasticJifeParameter) where T<:Number
...
- `stoparam::`[`StochasticJifeParameter`](@ref) : stochastic ogs parameter.
"""
function StochtoCP(x::T,stoparam::StochasticJifeParameter) where T<:Number
	return lin_func(x, stoparam.lower_bound, -1.0, stoparam.upper_bound, 1.0)
end

function pdf(stoparam::StochasticJifeParameter, x::Float64)
...
function pdf(stoparams::Vector{StochasticJifeParameter}, x)
	return foldl(*,map((x,y)->pdf(x,y),stoparams,x))
end

function truncated_randfun(ogsuqparams::JifeUQParams)
