...
function show(io::IO, mime::MIME"text/plain", stochparams::AbstractArray{StochasticJifeParameter})
	N = length(stochparams)
	title = "StochasticJifeParameters"
...
function show(io::IO, ogsuqparams::JifeUQParams)
	println(io)
	printstyled(io, "JifeUQParams"; color = :light_white)
	println(io)
	show(io, ogsuqparams.stochasticmodelparams)
	show(io, ogsuqparams.samplemethodparams)
end

function show(io::IO, ogsuq::JifeUQASG)
	println(io)
	printstyled(io, "JifeUQASG"; color = :light_white)
	println(io)
...
function show(io::IO, ogsuq::T) where T<:AbstractJifeUQMonteCarlo
	println(io)
